<template>
  <div class="flex-col">
    ...
    <p class="text-4xl text-center bold mt-5">HOLIDAY PACKAGES</p>
        ...
        <div class="flex flex-col justify-between w-[400px]">
          ...
          >
            <div
              ...
              "
            >
              {{ p.attributes.city.data.attributes.name.toUpperCase() }}
            </div>
          </div>
          <div class="p-5">
            <div class="flex justify-between">
              <div class="h-[5rem]">
                <div class="text-2xl pt-0 mt-0">
                  {{ p.attributes.Title.toUpperCase() }}
                </div>
              </div>
              <div class="flex items-start text-lime-500 pt-2">
                <div
                  v-for="(star, index) in getArray(p.attributes.star_rating)"
                  :key="index"
                >
                  <IconStar />
                </div>
            ...
            <div class="text-center">
              Valid: {{ p.attributes.from }} to {{ p.attributes.To }}
            </div>
            <div class="text-2xl text-center text-lime-500">
              {{ randsFunction(p.attributes.price_cpt) }}
            </div>
            <div class="text-gray-500">
              {{ excerptFunc(p.attributes.description) }}
            </div>
